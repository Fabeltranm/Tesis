...
\begin{resumen}
  En esta sección se abarcan los conceptos básicos de anatomía  y
  fisiología cardiaca, dando un repaso por las arritmias cardiacas, en especial
  la fibrilación. Luego se pasa a los módelos matemáticos que describen
  la actividad electrica de la células y el tejido  cardiaco en 2D y 3D
  realistas. A posteriori se analizan los diferentes módelos para la generación
  y propagación de la actividad eléctrica del miocardio. se hace una
  descripción del módelo autómata Celular propuesto en \cite{Alonso-Atienza05},
  como un módelo de baja carga computacional. se presenta el módelo  \ac{LRd}
  ...
  concluir el capitulo, se hace una introducción a los sistema
  de medida simulados, en donde se plantean la definicion de \ac{MSD}, usada a posteriori
  para el cálculo de volumen equivalente. Está da un acercando a la simulación
  cardiacas que se trabaja en esta Tesis.  
  
  
  TODO: habar de   velocidad de conducción que sera usada en el autómata celular
...
Cuando se presentan alguna alteración a este ciclo cardiaco, bien sea por
alteraciones en la generación y/o en la conducción de los impulsos eléctricos, se habla de
transtornos cardiacos, conocidos como arritmias. Por lo tanto, el registro de la
actividad eléctrica, mediante sistemas de electrodos intracardiacos \ac{EGM} o en la superficie del corazón \ac{ECG},
proporciona informacion sobre el estado normal o anormal del ciclo cardiaco.
...
se encuentra al interior del tórax y contiguo a los pulmones. Se encuentra
recubierto por el saco pericárdiaco, y acorde con el sistema  circulatorio
...
permite bombear la sangre al cuerpo, sístole auricular y ventricular
y  la diástole, movimientos de contracción y dilatación respectivamente. la
generación de estos eventos cardiacos, el músculo cardiaco se autoestimula y
la secuencia de las contracciones cardiacas son producida  por la
despolarización de las celulas cardiacas, y  propagada por el miocardio, a través del sistema de conducción del corazón.
...
medio intracelular está a menor potencial que el extracelular. La membrana es
 ...
 decir, un cambio en el potencial eléctrico transmembrana. cuando la celula esta
...
la ausencias de la fase de reposo, vista en el \ac{APD} de las celulas ventriculares. 


Las celulas cardiacas tiene la propiedad de excitabilidad, funcion de \ac{APD}
...
es el tiempo en el que la célular exitable no responde a un estimula externo,
...
propagación del \ac{AP} \ac{jalife2011}.
...
en la figura \ref{fig:circuitomembrana}. el módelo estas  basado en las curvas de corrietne 
nedidas en unsa celula nerviosa, por lo cual no se evidencia la contribución de
...
y con mayor complejida. En \cite{Sachse04}, se compila una listado de diversos
autores, que han modelados los miocitos. Los primeros trabajos basados en
el módelo de Hodgkin-Huxleym, y que dan inicio al módelo de celulas
...
celulas diferenten, epicardio, endocardio  y miocardio. este estudio se
...
formalismo matemático que modelar la electrofisiología de una celula ventricular
del perro, \ac{HRd}.
 ...
 varian entre cada especie \ref{OHara2012}, y son de  importnacia en el estudo
...
caracteristicas del \ac{AP}, en el autómata celular propuesta en 
\cite{Alonso-Atienza05} la celular puede adopta 3 estados diferentes:


\begin{itemize}
\item Reposo: periodo  durante el cual la celula esta relaja  y puede ser
...
De esta manera, el periodo refractario 1 y 2 es el tiempo  en el cual la celula
...
tasa de estimulación a la que este sometida la membrana célular. Al tener una
...
bajo. De esta manera, en el autómata celular el tiempo es el estado en reposo es
...
la celula es en estado Refractaio 1 es una fracción del tiempo total del
\ac{APD}, por lo general el 10\%, y el restatan 90\% es, por lo tanto, el tiempo
que dura la célula en el estado refractario 2. Es asi, como los tienpo de
repolaizacion total y parcial se fijan acorde al valor de \ac{APD} predecesor.

En cuanto a la despolarización del micito, en el autámata celular se da en
función de la probabilidad de excitación de la celula cardiaca. 
...
autores han modelado el sistema de propagación de excitación del corazón, por
medio del autómata celular. La ventaja de simular la dinámica cardiaca, por
medio del autómata celular, es la eficiencia computacional que ofrece la
versatilidad de variar las propiedades de conducción. En general, el módelo
automata celular de la membrana esta conformado en primera instancia, como se
observa en la Figura \ref{fig:automata1},  por la red discreta de nodos que
representa la estructura espacial del tejido cardiaco, y en segunda instancia, la representación del autómata celular de cada
nodo que representan los miocitos. Por lo tanto, se dice que cada miosito,
cuenta con un número $N$ de celulas conectadas y de esta forma se
conforma la región de vecindad  usada para  estimar la propogación del \ac{AP}.
la comunicación entre las celulas vecinas  es de forma determinista, uniforme  y
sincrónica.

En esta \nombreDoc, el módelo del tejido cardiaca, se basa en el 
trabajo realzado en \cite{Alonso-Atienza05}, donde se concibe el musculo
cardíaco como un conjunto de elementos (miocitos) discretos interconectados
entre si. y el \ac{AP} de cada celula es representada por 3 estados posibles:
Reposo, refractario 1 y 2. El cambio de  de los estados de cada célula, viene
dado en función del estado de los elementos vecinos y de  acuerdo al estado
predecesor.
...
\caption{Represetación del módelo del autómata celular. (a)\ac(AP) es modelado
acorde a las fases de despolarización de la celula y los tres estados del
...
forma de cruz  y esta commpuesta por 5 celdas. (c) representación de la
...
\subsubsection{módelo de reaccion-difusión}
 ...
 Denominados de reacción-difusión, reproducen la dinámica de membrana simulando
 el paso de las corrientes iónicas a través de los canales iónicos y el
 potencial de membrana asociado16-18. La conexión entre las células se modela
 mediante resistencias eléctricas y la propagación del impulso se calcula
 resolviendo las ecuaciones que gobiernan los circuitos eléctricos19,20. Estos
 módelos son mucho más realistas y reproducen mejor las situaciones complejas,
 pero requieren una carga computacional muy elevada y a menudo son necesarias
 muchas horas de cálculo en multiprocesadores para poder simular 1 o 2 segundos
 de actividad eléctrica21.
